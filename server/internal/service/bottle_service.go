...
func (s *bottleService) ReReleaseBottle(ctx context.Context, input ReReleaseBottleInput) (*domain.Bottle, error) {
	...
	// Tests use in-memory repositories without a database pool. Production
	// always takes the transaction path.
	if s.pool == nil {
		err = apply(nil)
	} else {
	...
	// No broadcast during Mystery Delay. The scheduler announces the Cork when
