...
Done (2026-07-27): `POST /api/v1/bottles/:id/stamp` now verifies Turnstile and
the strict IP limit, sanitizes a seal and/or ≤80-character note, and appends one
durable Stamp event without moving or claiming the Bottle. The Open view can add
a Stamp and immediately shows it in the Journey. Demo: Open a Cork, choose a seal
or write a note, then select “Add Stamp”; server coverage runs with
`go test ./...`.
